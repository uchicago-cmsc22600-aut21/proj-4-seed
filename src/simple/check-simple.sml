...
structure CheckSimple : sig
  ...
  (* `check (msg, chkCensus, prog)` checks the SimpleAST program for a number of invariants,
   * such as that every variable is used has a binding, variables do not have multiple
   * bindings, and that primitive operations are applied to the correct number of arguments.
   * If the `chkCensus` flag is true, then it also checks that census counts are correct.
   * It reports error to the stdErr stream and will return `true` if there were errors and
   * `false` otherwise.
   *)
        ...
        (* check that a variable use occurs in its scope and bump the use counter *)
          ...
          and chkRHS (lhsCxt, bnd, rhs) = (case rhs
                 of S.R_EXP e => chkExp (bnd, e)
                  ...
                  | S.R_DCON(dc, v) =>
                      chkValue (bnd, conappCxt(dc, v)) v
                (* end case *))
          and chkRule bnd (pat, exp) = chkExp (chkPat (bnd, pat), exp)
          and chkPat (bnd, pat) = (case pat
                 of S.P_DCON(dc, x) => chkVarBnd (conapppatCxt (dc, x)) (x, bnd)
                  | S.P_CONST _ => bnd
