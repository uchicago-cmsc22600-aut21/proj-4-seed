...
structure DumpSimpleAST : sig
  ...
  (* `dumpToFile suffix (stem, simple)` dumps the SimpleAST to the file `stem`.`suffix` *)
   ...
   *)
    structure ConIdGen = IdGen (DC.Tbl)
    structure VarIdGen = IdGen (V.Tbl)
    ...
    fun assignIds (T.PROG(vArgs, body)) = let
          ...
          fun doExp (T.E(_, e)) = (case e
                 of T.E_LET(x, rhs, e) => (assignVarId x; doRHS rhs; doExp e)
                  ...
                  | T.E_COND(tst, vs, e1, e2) => (List.app doValue vs; doExp e1; doExp e2)
          ...
          and doRHS (T.R_EXP e) = doExp e
            ...
            | doRHS (T.R_DCON(dc, v)) = (assignConId dc; doValue v)
          and doRule (p, e) = (doPat p; doExp e)
          and doPat (T.P_DCON(dc, x)) = (assignConId dc; assignVarId x)
            | doPat (T.P_CONST dc) = assignConId dc
  ...
  (* helper functions to construct the S-Expression `(sym ...)` *)
    ...
    val mkConAppPat = mkNode "P_DCON"
    val mkConPat = mkNode "P_CONST"
  ...
  (* convert basic values to an S-expression *)
    ...
    and exp2sexp (env : env) (T.E(_, e)) = (case e
           of T.E_LET(x, rhs, e) => mkLetExp [
            ...
            | T.E_COND(tst, vs, e1, e2) => mkCondExp [
    ...
    and rhs2sexp (env : env) rhs = (case rhs
           of T.R_EXP e => exp2sexp env e  (* don't bother with a node for this case *)
            ...
            | T.R_DCON(dc, v) => mkConAppRHS [con2sexp env dc, value2sexp env v]
          (* end case *))

    and rule2sexp (env : env) (p, e) = LIST[pat2sexp env p, exp2sexp env e]

    and pat2sexp (env : env) p = (case p
           of T.P_DCON(dc, x) => mkConAppPat [con2sexp env dc, var2sexp env x]
            | T.P_CONST dc => mkConPat (con2list env dc)
