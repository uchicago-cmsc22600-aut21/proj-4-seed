...
structure IdProps : sig
      ...
      (* variable mapping *)
      val varSet : BindTree.VarId.t * Var.t -> unit
      val var : BindTree.VarId.t -> Var.t
    ...
    val {setFn = varSet, getFn = var, ...} = let
          fun mkVar (x : BT.VarId.t) : Var.t =
                raise Fail("IdProps.var: must set mapping for " ^ BT.VarId.nameOf x)
          in
            BT.VarId.newProp mkVar
          end
