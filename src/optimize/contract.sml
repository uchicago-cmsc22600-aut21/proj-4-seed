 ...
 *      - contractive inlining [EXTRA CREDIT]
 *)
...
structure Contract : sig
  ...
  (* interesting things that a variable might be bound to *)
    datatype bind
      ...
      | ConApp of DC.t * S.value        (* bound to a data constructor application *)
      | Func of S.var list * S.exp      (* bound to a single-use function [EXTRA CREDIT] *)
   ...
   *)
    ...
    fun bindVarsToVals ([], [], env) = env
      | bindVarsToVals (x::xs, v::vs, env) = bindVarsToVals (xs, vs, bindVarToVal(x, v, env))
  ...
  (* rename values by replacing variables with the value that they are bound to (if any) *)
    ...
    fun transform (S.PROG(vArgs, e)) = let
        ...
        (* smart constructor for let-bindings that does on-the-fly let floating *)
          fun mkLet (x, S.E(_, S.E_LET(y, rhs, e1)), e2) = (
            ...
            | mkLet (x, S.E(_, S.E_FUN(f, xs, body, e1)), e2) = (
                change();
                        ...
                        case bind
                         of Value(S.V_CON dc) => (
                              ...
                              case reduce (dc, rules)
                               of (S.P_CONST _, e) => xformExp(env, e)
                                | (S.P_VAR x, e) => xformExp(VMap.insert(env, x, bind), e)
                                | _ => raise Match
                              (* end case *))
                          | ConApp(dc, v') => (
                              change();
          ...
          and xformRHS (env, lhs, rhs) = (case rhs
                 of S.R_EXP e => raise Fail "impossible"
                  ...
                  | S.R_DCON(dc, v) => let
                      val v' = rn env v
                      in
                        RENAME(VMap.insert(env, lhs, ConApp(dc, v')), S.R_DCON(dc, v'))
                      end
                (* end case *))
