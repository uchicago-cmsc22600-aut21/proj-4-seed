...
structure Parser : sig
  ...
  (* parse a ML Lite program from an input stream using the given error
  ...
  (* glue together the lexer and parser *)
    structure MLLParser = MLLParseFn(MLLLex)
  ...
  (* parse a file, returning a parse tree *)
    fun parse (inStrm, errStrm) = let
          ...
          val lexer = MLLLex.lex (Error.sourceMap errStrm) (lexErr errStrm)
          val (yield, _, errs) = MLLParser.parse lexer (MLLLex.streamify get)
          in
