...
void _mll_invoke_gc (MLL_word_t *sp, size_t allocSz, uint32_t numRoots, MLL_word_t *roots)
{
  ...
  // swap spaces
    {
        MLL_semispace_t *tmp = _mll_heap._toSp;
        _mll_heap._toSp = _mll_heap._fromSp;
        _mll_heap._fromSp = tmp;
      // set _usedTop for both semispaces
        _mll_heap._toSp->_usedTop = _mll_heap._toSp->_baseAddr;
        _mll_heap._fromSp->_usedTop = (uint64_t)_mll_heap._allocPtr;
#ifdef GC_DEBUG
      // DEBUGGING
        fprintf(stderr, "\n\n------------- start GC -------------\n");
        fprintf(stderr, "from space = [%#llx .. %#llx)\n",
            _mll_heap._fromSp->_baseAddr, _mll_heap._fromSp->_usedTop);
        fprintf(stderr, "to space   = [%#llx .. %#llx)\n",
            _mll_heap._toSp->_baseAddr, _mll_heap._toSp->_baseAddr + _mll_heap._toSp->_szB);
#endif
    }
        ...
        while ((frame = lookup_return_address(StatePoints, *(uint64_t*)(stackPtr))) != 0) {
          ...
          // process pointers
            for (int i = 0; i < frame->numSlots; i++) {
                ...
                MLL_word_t *slotPtr = (MLL_word_t *)(stackPtr + slotInfo.offset);
#ifdef GC_DEBUG
                {
                    MLL_word_t w = *slotPtr;
                    MLL_word_t newW = forward_ptr(w);
                    *slotPtr = newW;
  ...
  // scan to-space
    {
        MLL_semispace_t *toSp = _mll_heap._toSp;

        MLL_word_t *p = (MLL_word_t *)toSp->_baseAddr;
        while (p < (MLL_word_t *)toSp->_usedTop) {
  ...
  // reset allocation and limit pointers
    _mll_heap._allocPtr = (MLL_word_t *)(_mll_heap._toSp->_usedTop);
    _mll_heap._limitPtr =
        (MLL_word_t *)(_mll_heap._toSp->_baseAddr + _mll_heap._toSp->_szB);
...
static MLL_word_t forward_ptr (MLL_word_t r)
{
    ...
    if ((_mll_heap._fromSp->_baseAddr <= (uint64_t)r)
    &&  ((uint64_t)r < _mll_heap._fromSp->_usedTop)) {
...
void _mll_init_heap ()
{
  ...
  // allocate to/from-space descriptors
    _mll_heap._toSp = (MLL_semispace_t *)malloc(sizeof(MLL_semispace_t));
    _mll_heap._fromSp = (MLL_semispace_t *)malloc(sizeof(MLL_semispace_t));

    _mll_heap._toSp->_baseAddr = (uint64_t)malloc(SEMISPACE_SZB);
    if (_mll_heap._toSp->_baseAddr == 0) {
        fprintf (stderr, "unable to allocate to-space\n");
        exit (1);
    }
    _mll_heap._toSp->_szB = SEMISPACE_SZB;
    _mll_heap._toSp->_usedTop = _mll_heap._toSp->_baseAddr;

    _mll_heap._fromSp->_baseAddr = (uint64_t)malloc(SEMISPACE_SZB);
    if (_mll_heap._fromSp->_baseAddr == 0) {
        fprintf (stderr, "unable to allocate from-space\n");
        exit (1);
    }
    _mll_heap._fromSp->_usedTop = _mll_heap._fromSp->_baseAddr;
    _mll_heap._fromSp->_szB = SEMISPACE_SZB;

  // initialize the allocation and limit pointers
    _mll_heap._allocPtr = (MLL_word_t *)_mll_heap._toSp->_baseAddr;
    _mll_heap._limitPtr =
        (MLL_word_t *)(_mll_heap._toSp->_baseAddr + _mll_heap._toSp->_szB);
