...
structure Context : sig
  ...
  (* the different kinds of environments used to resolve use occurrences of identifiers
   * in ML Lite programs.
   *)
    type tyvar_env = BindTree.tyvar AtomMap.map
    type tycon_env = BindTree.tycon AtomMap.map
    type dcon_env = BindTree.conid AtomMap.map
    type var_env = BindTree.varid AtomMap.map

  (* the contaxt captures both the syntactic and semantic context needed to check
   * the bindings of a ML Lite program
   *)
    type t
  ...
  (* lookup a data-constructor by name; `NONE` means that it is unbound in the context *)
  ...
  (* print the environments of a context to stdOut (for debugging) *)
    ...
    type var_env = BindTree.varid AMap.map
    ...
    fun setSpan (Cxt{errStrm, tvEnv, tycEnv, conEnv, varEnv, ...}, span) = Cxt{
            ...
            tvEnv=tvEnv, tycEnv=tycEnv, conEnv=conEnv, varEnv=varEnv
          }
    ...
    fun new errStrm = let
          ...
          fun insVar (x, env) = AMap.insert(env, Atom.atom(BT.VarId.nameOf x), x)
          in
            Cxt{
                errStrm = errStrm,
                span = (0, 0),
                ...
                conEnv = List.foldl insCon AMap.empty [
                    ...
                    B.conNil    (* we omit "::" because it is a special syntactic form *)
                  ],
    ...
    fun dump (Cxt stuff) = let
          ...
          in
            ...
            prEnv "varEnv" (prId BT.VarId.toString) (#varEnv stuff);
            print "#####\n"
          end
