 ...
 *
 * Support for that last few stages of compilation
 *)

structure UnixUtil : sig

  (* run the specified Unix command with the given arguments.  The command must be in
   * the PATH.  If the command is not found or fails, an error message is printed
  ...
  (* the section for the stack map is OS specific *)
                         ...
                         of SOME ln => let
                              val seenMap = seenMap orelse
                                    if String.isPrefix "__LLVM_StackMaps:" ln
                                      then (
                                        TextIO.output(outS, "\t.globl __LLVM_StackMaps\n");
                                        true)
                                      else false
                              in
    ...
    fun runtimePath () = let
          ...
          val objPath = Path.joinDirFile{dir = rtDir, file = "mll-rt.o"}
