...
structure CodeGenInfo : sig
  ...
  (* return the global for the given function label *)
    fun funcGlobal lab = (case CFGLabel.kindOf lab
           of CFGLabel.LK_Entry func =>
                LG.new(CFGFunct.nameOf func, Util.cvtType(CFGFunct.typeOf func))
            | _ => raise Fail(concat[
...
(* FIXME: should not be true for entry fuction, but we fix it in the LLVM library *)
                  tailCC = true,
                  retTy = Util.cvtArgType(CFGFunct.returnTypeOf func),
